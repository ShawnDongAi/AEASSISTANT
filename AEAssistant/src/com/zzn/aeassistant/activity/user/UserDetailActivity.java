...
public class UserDetailActivity extends BaseActivity {
	...
	private View layoutHead, userHeadIcon, layoutProject,
			/* layoutIDCard, layoutIDCardImg, */ btnCall, layoutRate,
			rateIcon, /* diverIDCard, diverIDCardImg, idCardIcon, */
	...
	protected void initView() {
		Node node = null;
		try {
			node = (Node) getIntent().getSerializableExtra(CodeConstants.KEY_PROJECT_VO);
		} catch (Exception e) {
			finish();
		}
		if (node == null || node.getData() == null || !(node.getData() instanceof ProjectVO)) {
			finish();
		}
		...
		if (projectVO.getPARENT_ID().equals(projectID) || projectVO.getPROJECT_ID().equals(projectID)) {
		...
		if (!StringUtil.isEmpty(projectVO.getCREATE_USER_HEAD())) {
			imageLoader.displayImage(String.format(URLConstants.URL_IMG, projectVO.getCREATE_USER_HEAD()), head,
					options);
		}
		new GetUserTask().execute(projectVO.getCREATE_USER());
		registerReceiver(userInfoReceiver, new IntentFilter(ACTION_UPDATE_IDCARD_IMG));
		initMenu();
	}
	...
	private void initMenu() {
		...
		headMenu = new PopupWindow(menuView, LayoutParams.MATCH_PARENT, LayoutParams.MATCH_PARENT);
		headMenu.setBackgroundDrawable(getResources().getDrawable(R.color.transparent_lightslategray));
	...
	public void onClick(View v) {
		...
		case R.id.user_layout_project:
			...
			projectintent.putExtra(CodeConstants.KEY_TITLE, getString(R.string.modify_project_name));
			projectintent.putExtra(CodeConstants.KEY_DEFAULT_TEXT, projectVO.getPROJECT_NAME());
			projectintent.putExtra(CodeConstants.KEY_HINT_TEXT, getString(R.string.project_hint_name));
		...
		case R.id.user_layout_idcard:
			if (userVO != null) {
				...
				intent.putExtra(CodeConstants.KEY_TITLE, getString(R.string.modify_user_idcard));
				intent.putExtra(CodeConstants.KEY_DEFAULT_TEXT, idcard.getText().toString());
				intent.putExtra(CodeConstants.KEY_HINT_TEXT, getString(R.string.hint_idcard));
				intent.putExtra(CodeConstants.KEY_SINGLELINE, true);
				intent.putExtra(CodeConstants.KEY_INPUT_TYPE, InputType.TYPE_NUMBER_FLAG_SIGNED);
		...
		case R.id.user_layout_idcard_img:
			if (userVO != null) {
				...
				intent.putExtra(CodeConstants.KEY_IDCARD_FRONT, userVO.getIDCARD_FRONT());
				intent.putExtra(CodeConstants.KEY_IDCARD_BACK, userVO.getIDCARD_BACK());
				intent.putExtra(CodeConstants.KEY_IDCARD_HAND, userVO.getIDCARD_HAND());
		...
		case R.id.btn_call:
			try {
				Intent intent = new Intent(Intent.ACTION_DIAL, Uri.parse("tel:" + projectVO.getCREATE_USER_PHONE()));
				startActivity(intent);
			} catch (Exception e) {
		...
		case R.id.user_layout_workspace:
			...
			intent.putExtra(CodeConstants.KEY_PROJECT_ID, projectVO.getPROJECT_ID());
	...
	private class CheckRateTask extends AsyncTask<Node, Integer, Boolean> {
		...
		protected Boolean doInBackground(Node... params) {
			...
			while (node != null && node.getId() != null && node.getParent() != null && node.getParent().getId() != null
	...
	private class GetUserTask extends AsyncTask<String, Integer, HttpResult> {
		...
		protected HttpResult doInBackground(String... params) {
			...
			HttpResult result = AEHttpUtil.doPost(URLConstants.URL_QUERY_USER_BY_ID, param);
			return result;
		}
		...
		protected void onPostExecute(HttpResult result) {
			...
			if (result.getRES_CODE().equals(HttpResult.CODE_SUCCESS)) {
				try {
					userVO = GsonUtil.getInstance().fromJson(result.getRES_OBJ().toString(), UserVO.class);
					sex.setText(!userVO.getSEX().trim().equals("1") ? R.string.male : R.string.female);
					...
					score.setText(getString(R.string.lable_score, userVO.getRATE() + ""));
				} catch (Exception e) {
					e.printStackTrace();
	...
	private class UpdateProjectTask extends AsyncTask<String, Integer, HttpResult> {
		@Override
		protected void onPreExecute() {
			super.onPreExecute();
			AEProgressDialog.showLoadingDialog(mContext);
		...
		protected HttpResult doInBackground(String... params) {
			...
			String param = "project_id=" + projectVO.getPROJECT_ID() + "&project_name=" + nameString;
			HttpResult result = AEHttpUtil.doPost(URLConstants.URL_UPDATE_PROJECT_NAME, param);
	...
	private class UpdateIDCardTask extends AsyncTask<String, Integer, HttpResult> {
		...
		protected HttpResult doInBackground(String... params) {
			...
			String param = "user_id=" + userVO.getUSER_ID() + "&idcard=" + idcardString;
		...
		protected void onPostExecute(HttpResult result) {
			...
			if (result.getRES_CODE().equals(HttpResult.CODE_SUCCESS)) {
				if (result.getRES_OBJ() != null && !StringUtil.isEmpty(result.getRES_OBJ().toString())) {
					...
					imageLoader.displayImage(String.format(URLConstants.URL_IMG, vo.getURL()), head, options);
					if (projectVO.getPROJECT_ID().equals(projectID)) {
