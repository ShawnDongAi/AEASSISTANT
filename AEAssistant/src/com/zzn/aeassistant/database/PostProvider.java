...
public class PostProvider extends ContentProvider {
	...
	static {
		...
		uriMatcher.addURI(PostDBHelper.AUTHORITY, "item/#",
				PostDBHelper.ITEM_ID);
		uriMatcher.addURI(PostDBHelper.AUTHORITY, "insert/#",
				PostDBHelper.ITEM_INSERT);
		uriMatcher.addURI(PostDBHelper.AUTHORITY, "new/#",
				PostDBHelper.ITEM_NEW);
	}
	...
	static {
		...
		postProjectionMap.put(PostDBHelper.PROJECT_NAME,
				PostDBHelper.PROJECT_NAME);
		postProjectionMap.put(PostDBHelper.ROOT_ID, PostDBHelper.ROOT_ID);
		postProjectionMap.put(PostDBHelper.ROOT_PROJECT_NAME,
				PostDBHelper.ROOT_PROJECT_NAME);
		postProjectionMap.put(PostDBHelper.TIME, PostDBHelper.TIME);
		postProjectionMap.put(PostDBHelper.SEND_USER_ID,
				PostDBHelper.SEND_USER_ID);
		postProjectionMap.put(PostDBHelper.SEND_USER_NAME,
				PostDBHelper.SEND_USER_NAME);
		postProjectionMap.put(PostDBHelper.SEND_PROJECT_ID,
				PostDBHelper.SEND_PROJECT_ID);
		postProjectionMap.put(PostDBHelper.SEND_PROJECT_NAME,
				PostDBHelper.SEND_PROJECT_NAME);
	...
	public Cursor query(Uri uri, String[] projection, String selection,
		...
		case PostDBHelper.ITEM_ID: {
			...
			sqlBuilder.setTables(PostDBHelper.table);
			sqlBuilder.setProjectionMap(postProjectionMap);
			sqlBuilder.appendWhere(PostDBHelper.USER_ID + "=" + user_id
					+ " or (" + PostDBHelper.SEND_USER_ID + "like %" + user_id
					+ "% and" + PostDBHelper.IS_PRIVATE + "='1')");
			break;
		}
		case PostDBHelper.ITEM_NEW: {
			String user_id = uri.getPathSegments().get(1);
			sqlBuilder.setTables(PostDBHelper.table);
			sqlBuilder.setProjectionMap(postProjectionMap);
			sqlBuilder.appendWhere(PostDBHelper.SEND_USER_ID + "like %"
					+ user_id + "% and " + PostDBHelper.IS_NEW + "='0'");
			break;
		}
		default:
			throw new IllegalArgumentException("Error Uri: " + uri);
		...
		Cursor cursor = sqlBuilder.query(db, projection, selection,
				...
				StringUtil.isEmpty(sortOrder) ? PostDBHelper.DEFAULT_SORT_ORDER
						: sortOrder, limit);
	...
	public Uri insert(Uri uri, ContentValues values) {
		if (uriMatcher.match(uri) != PostDBHelper.ITEM_INSERT) {
			throw new IllegalArgumentException("Error Uri: " + uri);
		}
		SQLiteDatabase db = AEApp.getDbHelper().getWritableDatabase(
				AESQLiteHelper.ENCRYPT_KEY);
		long id = db.insert(PostDBHelper.table, PostDBHelper.USER_ID, values);
		db.close();
		if (id < 0) {
	...
	public int update(Uri uri, ContentValues values, String selection,
		...
		switch (uriMatcher.match(uri)) {
		case PostDBHelper.ITEM: {
			count = db.update(PostDBHelper.table, values, selection,
					selectionArgs);
			break;
		}
		case PostDBHelper.ITEM_ID: {
			String id = uri.getPathSegments().get(1);
			count = db.update(PostDBHelper.table, values, PostDBHelper.POST_ID
					+ "="
					+ id
