...
public class PostDBHelper {
	...
	public static final String DB_CREATE = "create table " + table + " ("
			+ POST_ID + " varchar(32) NOT NULL UNIQUE," + USER_ID
	...
	public static void insertPostList(List<PostVO> postList) {
        ...
        for (PostVO postVo : postList) {
        	try {
    			...
    			values.put(PostDBHelper.USER_NAME,
    					postVo.getUser_name());
    			values.put(PostDBHelper.USER_HEAD,
    					postVo.getUser_head());
    			values.put(PostDBHelper.CONTENT, postVo.getContent());
    			values.put(PostDBHelper.ATTCH_ID, postVo.getAttch_id());
    			values.put(PostDBHelper.PROJECT_ID,
    					postVo.getProject_id());
    			values.put(PostDBHelper.PROJECT_NAME,
    					postVo.getProject_name());
    			values.put(PostDBHelper.ROOT_ID, postVo.getRoot_id());
    			values.put(PostDBHelper.ROOT_PROJECT_NAME,
    					postVo.getRoot_project_name());
    			values.put(PostDBHelper.TIME, postVo.getTime());
    			values.put(PostDBHelper.SEND_USER_ID,
    					postVo.getSend_user_id());
    			values.put(PostDBHelper.SEND_USER_NAME,
    					postVo.getSend_user_name());
    			values.put(PostDBHelper.SEND_PROJECT_ID,
    					postVo.getSend_project_id());
    			values.put(PostDBHelper.SEND_PROJECT_NAME,
    					postVo.getSend_project_name());
    			values.put(PostDBHelper.IS_PRIVATE,
    					postVo.getIs_private());
	...
	public static List<String> queryExistPost(String project_id) {
		...
		String sql = "select * from " + table + " where "
				+ PostDBHelper.PROJECT_ID + "='" + project_id + "' or "
