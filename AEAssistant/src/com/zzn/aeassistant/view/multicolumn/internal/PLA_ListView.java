package com.zzn.aeassistant.view.multicolumn.internal;
...
public class PLA_ListView extends PLA_AbsListView {
	...
	public PLA_ListView(Context context, AttributeSet attrs, int defStyle) {
		...
		if (a.hasValue(R.styleable.ListView_plaContentBackground)) {
			int contentBackgroundColor = a.getColor(
					R.styleable.ListView_plaContentBackground, 0);
	...
	public boolean removeHeaderView(View v) {
		if (mHeaderViewInfos.size() > 0) {
			...
			if (mAdapter != null
					&& ((PLA_HeaderViewListAdapter) mAdapter).removeHeader(v)) {
	...
	protected void fillGap(boolean down) {
		...
		if (down) {
			fillDown(mFirstPosition + count, getFillChildBottom()
					+ mDividerHeight);
		} else {
			...
			if (topOffset > 0) {
				if (lastPosition < mItemCount - 1 || lastBottom > end) {
					...
					if (lastPosition < mItemCount - 1) {
						...
						// with more rows, if
						// possible
						fillDown(lastPosition + 1, getFillChildBottom()
								+ mDividerHeight);
						// Close up the remaining gap
						adjustViewsUpOrDown();
					}
	...
	private void drawContentBackground(Canvas canvas) {
		if (getHeaderViewsCount() > 0) {
			...
			if (isHeaderView(firstVisibleView)) {
				View lastHeader = mHeaderViewInfos
						.get(mHeaderViewInfos.size() - 1).view;
